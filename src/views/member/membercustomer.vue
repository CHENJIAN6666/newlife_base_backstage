<template>
  <div class="app-container">
    ...
    <el-row  style="padding-bottom: 5px;">
      ...
      <el-button size="mini" type="primary" icon="plus" @click="showCreate" v-if="hasPerm('membercustomer:add')">添加
      </el-button>
      </el-col>
    </el-row>
    </div>
      ...
      <el-table-column align="center" label="序号" width="50">
        <template slot-scope="scope">
          <span v-text="getIndex(scope.$index)"> </span>
				 ...
				 <el-table-column align="center" prop="memberCustomerID" label="" width="40"></el-table-column>
			
				 <el-table-column align="center" prop="memberID" label="平台会员ID" width="100"></el-table-column>
			
				 <el-table-column align="center" prop="customerID" label="客户档案ID" width="100"></el-table-column>
			
				 <el-table-column align="center" prop="salesOrgID" label="归属部门" width="80"></el-table-column>
			
       
        
      <el-table-column align="center" label="操作" width="150" v-if="hasPerm('membercustomer:update')">
        <template slot-scope="scope">
    ...
    <el-dialog 
      ...
      <el-form size="mini" class="small-space" 
        ...
        label-position="right" label-width="150px" style='margin-left:50px;'>
			 ...
			 <el-col :span="24">
					<el-form-item label=""  prop="memberCustomerID" >
											<el-input type="text" v-model="tempModel.memberCustomerID">
											</el-input>
										</el-form-item>	
					...
					<el-form-item label="客户档案ID"  prop="customerID" >
											<el-input type="text" v-model="tempModel.customerID">
											</el-input>
										</el-form-item>	
			
					<el-form-item label="归属部门"  prop="salesOrgID" >
											<el-input type="text" v-model="tempModel.salesOrgID">
											</el-input>
										</el-form-item>	
...
<script>
  export default {
      ...
      return {
        ...
        rules: {
		  ...
		  { required: true, message: '请填写平台会员ID', trigger: 'blur' }],
			 ...
			 customerID:[
		  { required: true, message: '请填写客户档案ID', trigger: 'blur' }],
		
			 salesOrgID:[
		  { required: true, message: '请填写归属部门', trigger: 'blur' }]
    ...
    methods: {
      getList() {
        //查询列表
        if (!this.hasPerm('membercustomer:list')) {
          return
        }
        this.listLoading = true;
        this.api({
          url: "/membercustomer/listMembercustomer",
          method: "get",
      ...
      showCreate() {
        ...
        //显示新增对话框
					 this.tempModel.memberCustomerID = "";
					 this.tempModel.memberID = "";
					 this.tempModel.customerID = "";
					 this.tempModel.salesOrgID = "";
      ...
      showUpdate($index) {
        ...
        this.api({
          url: "/membercustomer/getMembercustomer",
          method: "post",
          data: {memberCustomerID:this.list[$index].memberCustomerID}
        }).then(data => {
          this.tempModel = data;
      ...
      createModel() {
        this.$refs['tempModel'].validate((valid) => {
          if (valid) {
            //保存新文章
            this.api({
              url: "/membercustomer/addMembercustomer",
              method: "post",
              data: this.tempModel
            }).then(() => {
              this.getList();
      ...
      updateModel() {
        this.$refs['tempModel'].validate((valid) => {
          if (valid) {
            //修改文章
            this.api({
              url: "/membercustomer/updateMembercustomer",
              method: "post",
              data: this.tempModel
            }).then(() => {
                let msg = "修改成功！";
      ...
      deleteModel($index) {
        this.$confirm("您确定要删除吗?", "提示", {
          ...
          this.api({
            url: "/membercustomer/deleteMembercustomer",
            method: "post",
